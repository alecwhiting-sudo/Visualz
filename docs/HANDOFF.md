...
| **Rapid double switch same frame** (A→B→C at frame N) | Both events at N; player applies both at `applyUpTo(N)`. Second capture reads the surface, which still holds **A's frame N** (B never rendered between `performSwitch` calls). So C ingests A's frame; B is built and disposed without rendering. Wasteful but deterministic and identical live-vs-replay. |
